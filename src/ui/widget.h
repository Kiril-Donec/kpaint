...
public:
  ...
  Widget* firstChild() { return (hasChildren() ? m_children.front() : nullptr); }
  Widget* lastChild() { return (hasChildren() ? m_children.back() : nullptr); }
  // Returns the next or previous siblings.
  Widget* nextSibling();
  Widget* previousSibling();
  Widget* pick(const gfx::Point& pt, const bool checkParentsVisibility = true) const;
  ...
  void getTextIconInfo(gfx::Rect* box,
  ...
  void closeWindow();
  void broadcastMouseMessage(const gfx::Point& screenPos, WidgetsList& targets);
  // ===============================================================
  // SIZE & POSITION
  // ===============================================================
  gfx::Size sizeHint();
  ...
  int mnemonic() const { return (m_mnemonic & kMnemonicCharMask); }
  bool mnemonicRequiresModifiers() const
  {
    return (m_mnemonic & kMnemonicModifiersMask ? true : false);
  }
  void setMnemonic(const int mnemonic, const bool requireModifiers);
  // Assigns mnemonic from the character preceded by the given
  // escapeChar ('&' by default).
  void processMnemonicFromText(const int escapeChar = '&', const bool requireModifiers = true);
...
protected:
  ...
  virtual void onBroadcastMouseMessage(const gfx::Point& screenPos, WidgetsList& targets);
