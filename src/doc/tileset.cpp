...
Tileset::Tileset(Sprite* sprite, const Grid& grid, const tileset_index ntiles)
...
void Tileset::remap(const Remap& remap)
{
  ...
  for (tile_index ti = 1; ti < size(); ++ti) {
    ...
    if (remap[ti] >= 0 && remap[ti] < m_tiles.size()) {
...
void Tileset::setTileData(const tile_index ti, const UserData& userData)
{
  if (ti >= 0 && ti < size())
    m_tiles[ti].data = userData;
}
void Tileset::set(const tile_index ti, const ImageRef& image)
{
  ASSERT(image);
...
tile_index Tileset::add(const ImageRef& image, const UserData& userData)
{
  ASSERT(image);
  ASSERT(image->width() == m_grid.tileSize().w);
  ASSERT(image->height() == m_grid.tileSize().h);
...
void Tileset::insert(const tile_index ti, const ImageRef& image, const UserData& userData)
{
  ASSERT(image);
  ASSERT(image->width() == m_grid.tileSize().w);
  ASSERT(image->height() == m_grid.tileSize().h);
...
void Tileset::setExternal(const std::string& filename, const tileset_index& tsi)
{
...
void Tileset::hashImage(const tile_index ti, const ImageRef& tileImage)
{
