...
static bool load_gui_config(os::WindowSpec& spec, bool& maximized)
{
 ...
 ifdef LAF_SKIA
  ...
  if (!screen) {
    std::printf("\n"
                "  KPaint cannot initialize GUI because it was compiled with LAF_BACKEND=none\n"
                "\n"
                "  Check the documentation in:\n"
                "  https://github.com/kpaint/laf/blob/main/README.md\n"
                "  https://github.com/kpaint/kpaint/blob/main/INSTALL.md\n"
                "\n");
    return false;
  }
 endif
  ...
  if (!frame.isEmpty()) {
    ...
    // e.g. this is needed in case that the user closed KPaint in a
    // 2nd monitor that then unplugged and start KPaint again.
    bool ok = false;
  ...
  if (frame.isEmpty()) {
    ...
    // Try to get Width/Height from previous KPaint versions
