...
typedef std::unique_ptr<Doc> DocPtr;
 ...
 define EXPECT_FRAME_ORDER(a, b, c, d)                                                             \
  EXPECT_TRUE(expect_frame(a, 0) && expect_frame(b, 1) && expect_frame(c, 2) && expect_frame(d, 3));
 define EXPECT_FRAME_ORDER6(a, b, c, d, e, f)                                                      \
  EXPECT_TRUE(expect_frame(a, 0) && expect_frame(b, 1) && expect_frame(c, 2) &&                    \
              expect_frame(d, 3) && expect_frame(e, 4) && expect_frame(f, 5));
 ...
 define EXPECT_CEL(y, x, v, u) EXPECT_TRUE(expect_cel(y, x, v, u));
 define EXPECT_EMPTY_CEL(y, x) EXPECT_TRUE(expect_empty_cel(y, x));
...
public:
  ...
  ~DocRangeOps() { doc->close(); }

protected:
  bool expect_layer(Layer* expected_layer, layer_t layer)
  {
    LayerList layers = sprite->allLayers();
    return expect_layer_frame(find_layer_index(layers, expected_layer), -1, layer, -1);
  }
  ...
  bool expect_layer_frame(layer_t expected_layer,
  ...
  bool expect_cel(layer_t expected_layer, frame_t expected_frame, layer_t layer, frame_t frame)
  {
    LayerList layers = sprite->allLayers();
    Cel* cel = layers[layer]->cel(frame);
    if (!cel)
      return false;
    color_t color = get_pixel(cel->image(), expected_frame, expected_layer);
    EXPECT_EQ(expected_color, color) << " - expecting layer " << expected_layer << " in " << layer
                                     << " and it is " << int(rgba_getr(color) / 10) << "\n"
                                     << " - expecting frame " << expected_frame << " in " << frame
                                     << " and it is " << int(rgba_getr(color) % 10);
  ...
  DocRange range(Layer* fromLayer,
                 frame_t fromFrNum,
                 Layer* toLayer,
                 frame_t toFrNum,
                 DocRange::Type type)
  ...
  DocRange range(layer_t fromLayer,
                 frame_t fromFrNum,
                 layer_t toLayer,
                 frame_t toFrNum,
                 DocRange::Type type)
  ...
  DocRange layers_range(Layer* layer) { return range(layer, -1, layer, -1, DocRange::kLayers); }
  ...
  DocRange cels_range(layer_t fromLayer, frame_t fromFrNum, layer_t toLayer, frame_t toFrNum)
  {
...
TEST_F(DocRangeOps, MoveLayersNoOp)
{
  ...
  EXPECT_EQ(layers_range(layer1),
            move_range(doc.get(), layers_range(layer1), layers_range(layer1), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(layers_range(layer1),
            move_range(doc.get(), layers_range(layer1), layers_range(layer2), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(layers_range(layer4),
            move_range(doc.get(), layers_range(layer4), layers_range(layer4), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(layers_range(layer4),
            move_range(doc.get(), layers_range(layer4), layers_range(layer4), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(layers_range(layer4),
            move_range(doc.get(), layers_range(layer4), layers_range(layer3), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  // Move two layer to the same place
  EXPECT_EQ(
    layers_range(layer1, layer2),
    move_range(doc.get(), layers_range(layer1, layer2), layers_range(layer1), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer1, layer2),
    move_range(doc.get(), layers_range(layer1, layer2), layers_range(layer1), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer1, layer2),
    move_range(doc.get(), layers_range(layer1, layer2), layers_range(layer2), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer1, layer2),
    move_range(doc.get(), layers_range(layer1, layer2), layers_range(layer2), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer1, layer2),
    move_range(doc.get(), layers_range(layer1, layer2), layers_range(layer3), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer3, layer4),
    move_range(doc.get(), layers_range(layer3, layer4), layers_range(layer2), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer3, layer4),
    move_range(doc.get(), layers_range(layer3, layer4), layers_range(layer3), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer3, layer4),
    move_range(doc.get(), layers_range(layer3, layer4), layers_range(layer3), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer3, layer4),
    move_range(doc.get(), layers_range(layer3, layer4), layers_range(layer4), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(
    layers_range(layer3, layer4),
    move_range(doc.get(), layers_range(layer3, layer4), layers_range(layer4), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  // Move four layers
  ...
  for (int i = 0; i < 2; ++i) {
    for (int layer = 0; layer < 4; ++layer) {
      EXPECT_EQ(
        ...
        move_range(doc.get(), layers_range(layer1, layer4), layers_range(layer), places[i]));
      EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
      EXPECT_FALSE(doc->undoHistory()->canUndo());
    }
    ...
    EXPECT_EQ(
      layers_range(layer1, layer4),
      move_range(doc.get(), layers_range(layer1, layer4), layers_range(layer1, layer4), places[i]));
    EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
    EXPECT_FALSE(doc->undoHistory()->canUndo());
  }
}
TEST_F(DocRangeOps, MoveFramesNoOp)
{
  ...
  EXPECT_EQ(frames_range(0),
            move_range(doc.get(), frames_range(0), frames_range(0), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(0),
            move_range(doc.get(), frames_range(0), frames_range(1), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(3),
            move_range(doc.get(), frames_range(3), frames_range(3), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(3),
            move_range(doc.get(), frames_range(3), frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(3),
            move_range(doc.get(), frames_range(3), frames_range(2), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  // Move two frame to the same place
  EXPECT_EQ(frames_range(0, 1),
            move_range(doc.get(), frames_range(0, 1), frames_range(0), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(0, 1),
            move_range(doc.get(), frames_range(0, 1), frames_range(0), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(0, 1),
            move_range(doc.get(), frames_range(0, 1), frames_range(1), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(0, 1),
            move_range(doc.get(), frames_range(0, 1), frames_range(1), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(0, 1),
            move_range(doc.get(), frames_range(0, 1), frames_range(2), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), frames_range(2, 3), frames_range(1), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), frames_range(2, 3), frames_range(2), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), frames_range(2, 3), frames_range(2), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), frames_range(2, 3), frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), frames_range(2, 3), frames_range(3), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_FALSE(doc->undoHistory()->canUndo());
  // Move four frames
  DocRangePlace places[] = { kDocRangeBefore, kDocRangeAfter };
  for (int i = 0; i < 2; ++i) {
    for (int frame = 0; frame < 4; ++frame) {
      EXPECT_EQ(frames_range(0, 3),
                move_range(doc.get(), frames_range(0, 3), frames_range(frame), places[i]));
      EXPECT_FRAME_ORDER(0, 1, 2, 3);
      EXPECT_FALSE(doc->undoHistory()->canUndo());
    }
    for (int frame = 0; frame < 3; ++frame) {
      EXPECT_EQ(
        frames_range(0, 3),
        move_range(doc.get(), frames_range(0, 3), frames_range(frame, frame + 1), places[i]));
      EXPECT_FRAME_ORDER(0, 1, 2, 3);
      EXPECT_FALSE(doc->undoHistory()->canUndo());
    }
    for (int frame = 0; frame < 2; ++frame) {
      EXPECT_EQ(
        frames_range(0, 3),
        move_range(doc.get(), frames_range(0, 3), frames_range(frame, frame + 2), places[i]));
      EXPECT_FRAME_ORDER(0, 1, 2, 3);
      EXPECT_FALSE(doc->undoHistory()->canUndo());
    }
    EXPECT_EQ(frames_range(0, 3),
              move_range(doc.get(), frames_range(0, 3), frames_range(0, 3), places[i]));
    EXPECT_FRAME_ORDER(0, 1, 2, 3);
    EXPECT_FALSE(doc->undoHistory()->canUndo());
  }
}
TEST_F(DocRangeOps, MoveCelsNoOp)
{
  EXPECT_EQ(cels_range(0, 0, 1, 1),
            move_range(doc.get(), cels_range(0, 0, 1, 1), cels_range(0, 0, 1, 1), kDocRangeAfter));
  EXPECT_CEL(0, 0, 0, 0);
  EXPECT_CEL(0, 1, 0, 1);
  EXPECT_CEL(1, 0, 1, 0);
  EXPECT_CEL(1, 1, 1, 1);
...
TEST_F(DocRangeOps, MoveLayers)
{
  ...
  EXPECT_EQ(layers_range(layer1),
            move_range(doc.get(), layers_range(layer1), layers_range(layer2), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer2, layer1, layer3, layer4);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  // One layer at the bottom
  EXPECT_EQ(layers_range(layer2),
            move_range(doc.get(), layers_range(layer2), layers_range(layer1), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer2, layer1, layer3, layer4);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  ...
  EXPECT_EQ(layers_range(layer2),
            move_range(doc.get(), layers_range(layer2), layers_range(layer4), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer3, layer4, layer2);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  // Move one layers before other
  EXPECT_EQ(layers_range(layer2),
            move_range(doc.get(), layers_range(layer2), layers_range(layer4), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer1, layer3, layer2, layer4);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  EXPECT_EQ(
    layers_range(layer1),
    move_range(doc.get(), layers_range(layer1), layers_range(layer3, layer4), kDocRangeBefore));
  EXPECT_LAYER_ORDER(layer2, layer1, layer3, layer4);
  doc->undoHistory()->undo();
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  ...
  EXPECT_EQ(
    ...
    move_range(doc.get(), layers_range(layer2, layer3), layers_range(layer4), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer4, layer2, layer3);
  EXPECT_EQ(
    layers_range(layer2, layer3),
    move_range(doc.get(), layers_range(layer2, layer3), layers_range(layer1), kDocRangeAfter));
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  // Move three layers at the bottom (but we cannot because the bottom is a background layer)
  layer1->setBackground(true);
  EXPECT_THROW(
    { move_range(doc.get(), layers_range(layer2, layer4), layers_range(layer1), kDocRangeBefore); },
    std::exception);
  EXPECT_LAYER_ORDER(layer1, layer2, layer3, layer4);
  layer1->setBackground(false);
  ...
  EXPECT_EQ(
    ...
    move_range(doc.get(), layers_range(layer1, layer3), layers_range(layer4), kDocRangeAfter));
...
TEST_F(DocRangeOps, MoveFrames)
{
  ...
  EXPECT_EQ(frames_range(1),
            move_range(doc.get(), frames_range(0), frames_range(1), kDocRangeAfter));
...
TEST_F(DocRangeOps, MoveFramesNonAdjacent)
{
  // Move frames 0,2...
  DocRange from;
  from.startRange(nullptr, 0, DocRange::kFrames);
  from.endRange(nullptr, 0);
  from.startRange(nullptr, 2, DocRange::kFrames);
  from.endRange(nullptr, 2);
  // Move frames 0,2 after 3...
  EXPECT_EQ(frames_range(2, 3), move_range(doc.get(), from, frames_range(3), kDocRangeAfter));
  EXPECT_FRAME_ORDER(1, 3, 0, 2);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move frames 0,2 before 3...
  EXPECT_EQ(frames_range(1, 2), move_range(doc.get(), from, frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_ORDER(1, 0, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move frames 0,2 before 0...
  EXPECT_EQ(frames_range(0, 1), move_range(doc.get(), from, frames_range(0), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 2, 1, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move frames 0,2 after 0...
  EXPECT_EQ(frames_range(0, 1), move_range(doc.get(), from, frames_range(0), kDocRangeAfter));
  EXPECT_FRAME_ORDER(0, 2, 1, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move frames 0,2 before 1...
  EXPECT_EQ(frames_range(0, 1), move_range(doc.get(), from, frames_range(1), kDocRangeBefore));
  EXPECT_FRAME_ORDER(0, 2, 1, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move frames 0,2 after 1...
  EXPECT_EQ(frames_range(1, 2), move_range(doc.get(), from, frames_range(1), kDocRangeAfter));
  EXPECT_FRAME_ORDER(1, 0, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Move 1,2,5...
  ...
  // Move 1,2,5 before 4...
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  EXPECT_EQ(frames_range(2, 4), move_range(doc.get(), from, frames_range(4), kDocRangeBefore));
  EXPECT_FRAME_ORDER6(0, 3, 1, 2, 5, 4);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,2,5 after 4...
  EXPECT_EQ(frames_range(3, 5), move_range(doc.get(), from, frames_range(4), kDocRangeAfter));
  EXPECT_FRAME_ORDER6(0, 3, 4, 1, 2, 5);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,2,5 before 2...
  EXPECT_EQ(frames_range(1, 3), move_range(doc.get(), from, frames_range(2), kDocRangeBefore));
  EXPECT_FRAME_ORDER6(0, 1, 2, 5, 3, 4);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,2,5 after 2...
  EXPECT_EQ(frames_range(1, 3), move_range(doc.get(), from, frames_range(2), kDocRangeAfter));
  EXPECT_FRAME_ORDER6(0, 1, 2, 5, 3, 4);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,2,5 before 1...
  EXPECT_EQ(frames_range(1, 3), move_range(doc.get(), from, frames_range(1), kDocRangeBefore));
  EXPECT_FRAME_ORDER6(0, 1, 2, 5, 3, 4);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,3,5...
  ...
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,3,5 before 5...
  EXPECT_EQ(frames_range(3, 5), move_range(doc.get(), from, frames_range(5), kDocRangeBefore));
  EXPECT_FRAME_ORDER6(0, 2, 4, 1, 3, 5);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  // Move 1,3,5 after 5...
  EXPECT_EQ(frames_range(3, 5), move_range(doc.get(), from, frames_range(5), kDocRangeAfter));
  EXPECT_FRAME_ORDER6(0, 2, 4, 1, 3, 5);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
}
TEST_F(DocRangeOps, MoveCels)
{
  ...
  move_range(doc.get(), cels_range(0, 0, 0, 0), cels_range(0, 1, 0, 1), ignore);
  EXPECT_CEL(0, 0, 0, 1);
  EXPECT_EMPTY_CEL(0, 0);
  doc->undoHistory()->undo();
  EXPECT_CEL(0, 0, 0, 0);
  move_range(doc.get(), cels_range(0, 0, 0, 1), cels_range(0, 2, 0, 3), ignore);
  EXPECT_CEL(0, 0, 0, 2);
  EXPECT_CEL(0, 1, 0, 3);
  EXPECT_EMPTY_CEL(0, 0);
  EXPECT_EMPTY_CEL(0, 1);
  doc->undoHistory()->undo();
  move_range(doc.get(), cels_range(0, 0, 1, 1), cels_range(2, 2, 3, 3), ignore);
  EXPECT_CEL(0, 0, 2, 2);
  EXPECT_CEL(0, 1, 2, 3);
  EXPECT_CEL(1, 0, 3, 2);
  EXPECT_CEL(1, 1, 3, 3);
  ...
  doc->undoHistory()->undo();
  move_range(doc.get(), cels_range(0, 0, 0, 3), cels_range(1, 0, 1, 3), ignore);
  EXPECT_CEL(0, 0, 1, 0);
  EXPECT_CEL(0, 1, 1, 1);
  EXPECT_CEL(0, 2, 1, 2);
  EXPECT_CEL(0, 3, 1, 3);
  ...
  move_range(doc.get(), cels_range(0, 0, 0, 2), cels_range(0, 1, 0, 3), ignore);
  EXPECT_CEL(0, 0, 0, 1);
  EXPECT_CEL(0, 1, 0, 2);
  EXPECT_CEL(0, 2, 0, 3);
  EXPECT_EMPTY_CEL(0, 0);
...
TEST_F(DocRangeOps, CopyFrames)
{
  ...
  EXPECT_EQ(frames_range(2),
            copy_range(doc.get(), frames_range(0), frames_range(2, 3), kDocRangeBefore));
  EXPECT_FRAME_COPY1(0, 1, 0, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(4),
            copy_range(doc.get(), frames_range(0), frames_range(2, 3), kDocRangeAfter));
  EXPECT_FRAME_COPY1(0, 1, 2, 3, 0);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(0),
            copy_range(doc.get(), frames_range(3), frames_range(0, 1), kDocRangeBefore));
  EXPECT_FRAME_COPY1(3, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(2),
            copy_range(doc.get(), frames_range(3), frames_range(0, 1), kDocRangeAfter));
  EXPECT_FRAME_COPY1(0, 1, 3, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  // Copy three frames
  EXPECT_EQ(frames_range(3, 5),
            copy_range(doc.get(), frames_range(0, 2), frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_COPY3(0, 1, 2, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(4, 6),
            copy_range(doc.get(), frames_range(0, 2), frames_range(3), kDocRangeAfter));
  EXPECT_FRAME_COPY3(0, 1, 2, 3, 0, 1, 2);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(0, 2),
            copy_range(doc.get(), frames_range(1, 3), frames_range(0), kDocRangeBefore));
  EXPECT_FRAME_COPY3(1, 2, 3, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(1, 3),
            copy_range(doc.get(), frames_range(1, 3), frames_range(0), kDocRangeAfter));
  EXPECT_FRAME_COPY3(0, 1, 2, 3, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(0, 2),
            copy_range(doc.get(), frames_range(0, 2), frames_range(0, 2), kDocRangeBefore));
  EXPECT_FRAME_COPY3(0, 1, 2, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(3, 5),
            copy_range(doc.get(), frames_range(0, 2), frames_range(0, 2), kDocRangeAfter));
  EXPECT_FRAME_COPY3(0, 1, 2, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
}
TEST_F(DocRangeOps, CopyFramesNonAdjacent)
{
  ...
  from.startRange(nullptr, 2, DocRange::kFrames);
  from.endRange(nullptr, 2);
  EXPECT_EQ(frames_range(3, 4), copy_range(doc.get(), from, frames_range(3), kDocRangeBefore));
  EXPECT_FRAME_COPY2(0, 1, 2, 0, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(4, 5), copy_range(doc.get(), from, frames_range(3), kDocRangeAfter));
  EXPECT_FRAME_COPY2(0, 1, 2, 3, 0, 2);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(2, 3), copy_range(doc.get(), from, frames_range(1), kDocRangeAfter));
  EXPECT_FRAME_COPY2(0, 1, 0, 2, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(1, 2), copy_range(doc.get(), from, frames_range(1), kDocRangeBefore));
  ...
  from.startRange(nullptr, 3, DocRange::kFrames);
  from.endRange(nullptr, 3);
  EXPECT_EQ(frames_range(0, 1), copy_range(doc.get(), from, frames_range(0), kDocRangeBefore));
  EXPECT_FRAME_COPY2(1, 3, 0, 1, 2, 3);
  doc->undoHistory()->undo();
  EXPECT_FRAME_ORDER(0, 1, 2, 3);
  EXPECT_EQ(frames_range(1, 2), copy_range(doc.get(), from, frames_range(0), kDocRangeAfter));
...
TEST(DocRangeOps2, DropInsideBugs)
{
  ...
  // layer2       layer2
  from.clearRange();
  from.selectLayer(layer3);
  to.clearRange();
  to.selectLayer(layer1);
  ...
  from.clearRange();
  from.selectLayer(layer2);
  to.clearRange();
  to.selectLayer(layer2);
  move_range(doc.get(), from, to, kDocRangeFirstChild);
  // Everything is exactly the same (no new undo operation)
  EXPECT_EQ(layer2, sprite->root()->layers()[0]);
  EXPECT_EQ(layer4, sprite->root()->layers()[1]);
...
TEST_F(DocRangeOps, MoveRangeWithTags)
{
  ...
  EXPECT_EQ(
    frames_range(0, 1),
    move_range(doc.get(), frames_range(1, 2), frames_range(0), kDocRangeBefore, kFitOutsideTags));
  ...
  EXPECT_EQ(
    frames_range(0, 1),
    move_range(doc.get(), frames_range(1, 2), frames_range(0), kDocRangeBefore, kFitInsideTags));
  EXPECT_FRAME_ORDER6(1, 2, 0, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(2, a->toFrame());
  EXPECT_EQ(3, b->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(1, 2),
    move_range(doc.get(), frames_range(0, 1), frames_range(2), kDocRangeAfter, kFitInsideTags));
  ...
  EXPECT_EQ(
    frames_range(1, 2),
    move_range(doc.get(), frames_range(0, 1), frames_range(2), kDocRangeAfter, kFitOutsideTags));
  EXPECT_FRAME_ORDER6(2, 0, 1, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(1, 2),
    move_range(doc.get(), frames_range(0, 1), frames_range(3), kDocRangeBefore, kFitOutsideTags));
  EXPECT_FRAME_ORDER6(2, 0, 1, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(0, a->toFrame());
  EXPECT_EQ(3, b->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(2, 3),
    move_range(doc.get(), frames_range(2, 3), frames_range(2), kDocRangeAfter, kFitOutsideTags));
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(2, 3),
    move_range(doc.get(), frames_range(2, 3), frames_range(3), kDocRangeBefore, kFitOutsideTags));
  EXPECT_FRAME_ORDER6(0, 1, 2, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(1, a->toFrame());
  EXPECT_EQ(4, b->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(4, 5),
    move_range(doc.get(), frames_range(0, 1), frames_range(5), kDocRangeAfter, kFitInsideTags));
  ...
  EXPECT_EQ(
    frames_range(4, 5),
    move_range(doc.get(), frames_range(0, 1), frames_range(5), kDocRangeAfter, kFitOutsideTags));
  EXPECT_FRAME_ORDER6(2, 3, 4, 5, 0, 1);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(0, a->toFrame());
  EXPECT_EQ(1, b->fromFrame());
  ...
  from.startRange(nullptr, 1, DocRange::kFrames);
  from.endRange(nullptr, 1);
  from.startRange(nullptr, 4, DocRange::kFrames);
  from.endRange(nullptr, 4);
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), from, frames_range(2), kDocRangeAfter, kFitOutsideTags));
  ...
  EXPECT_EQ(frames_range(2, 3),
            move_range(doc.get(), from, frames_range(3), kDocRangeBefore, kFitInsideTags));
  EXPECT_FRAME_ORDER6(0, 2, 1, 4, 3, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(1, a->toFrame());
...
TEST_F(DocRangeOps, CopyRangeWithTags)
{
  ...
  EXPECT_EQ(
    frames_range(0),
    copy_range(doc.get(), frames_range(1), frames_range(0), kDocRangeBefore, kFitOutsideTags));
  ...
  EXPECT_EQ(
    frames_range(0),
    copy_range(doc.get(), frames_range(1), frames_range(0), kDocRangeBefore, kFitInsideTags));
  EXPECT_FRAME_COPY3(1, 0, 1, 2, 3, 4, 5);
  EXPECT_EQ(0, a->fromFrame());
  EXPECT_EQ(3, a->toFrame());
  EXPECT_EQ(4, b->fromFrame());
  ...
  EXPECT_EQ(
    frames_range(3),
    copy_range(doc.get(), frames_range(1), frames_range(2), kDocRangeAfter, kFitOutsideTags));
