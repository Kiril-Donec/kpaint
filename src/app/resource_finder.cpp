...
void ResourceFinder::includeDataDir(const char* filename)
{
 ...
 ifdef _WIN32
  ...
  includeHomeDir(buf); // %AppData%/KPaint/data/filename
  includeBinDir(buf);  // $BINDIR/data/filename
 elif __APPLE__
  std::snprintf(buf, sizeof(buf), "data/%s", filename);
  includeUserDir(buf); // $HOME/Library/Application Support/KPaint/data/filename
  ...
  // $HOME/.config/kpaint/filename
  std::snprintf(buf, sizeof(buf), "kpaint/data/%s", filename);
  includeHomeConfigDir(buf);
  // $BINDIR/data/filename
  std::snprintf(buf, sizeof(buf), "data/%s", filename);
  includeBinDir(buf);
  // $BINDIR/../share/kpaint/data/filename (installed in /usr/ or /usr/local/)
  std::snprintf(buf, sizeof(buf), "../share/kpaint/data/%s", filename);
  includeBinDir(buf);
 endif
}
...
{
 ifdef _WIN32
  // %AppData%/KPaint/filename
  wchar_t* env = _wgetenv(L"AppData");
  if (env) {
    std::string path = base::join_path(base::to_utf8(env), "KPaint");
...
// else // Unix-like
  ...
  #ifdef __APPLE__
    // $HOME/Library/Application Support/KPaint/filename
    addPath(
      base::join_path(base::join_path(base::get_lib_app_support_path(), get_app_name()), filename)
        .c_str());
  #else // !__APPLE__
    // $HOME/.config/kpaint/filename
    includeHomeConfigDir((std::string("kpaint/") + filename).c_str());
  #endif
  }
// endif // end Unix-like
}
