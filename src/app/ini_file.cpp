...
{
  ResourceFinder rf;
  rf.includeUserDir("kpaint.ini");
  // getFirstOrCreateDefault() will create the KPaint directory
  // inside the OS configuration folder (~/.config/kpaint/, etc.).
  std::string fn = rf.getFirstOrCreateDefault();
 ifdef __APPLE__
  // On OS X we migrate from ~/.config/kpaint/* -> "~/Library/Application Support/KPaint/*"
  if (!base::is_file(fn)) {
    try {
      ...
      old_rf.includeHomeDir(".config/kpaint/kpaint.ini");
  ...
  // (~/.asepriterc -> ~/.config/kpaint/kpaint.ini)
  {
    ResourceFinder old_rf;
