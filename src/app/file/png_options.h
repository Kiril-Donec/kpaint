...
public:
  ...
  void addChunk(Chunk&& chunk) { m_userChunks.emplace_back(std::move(chunk)); }
  bool isEmpty() const { return m_userChunks.empty(); }
  int size() const { return int(m_userChunks.size()); }
